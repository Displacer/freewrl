 ...
 * 
 * $Id: jsVRMLClasses.c,v 1.15 2004/10/05 15:19:35 crc_canada Exp $
 * 
 */
...
doMFToString(JSContext *cx, JSObject *obj, const char *className, jsval *rval)
{
    ...
    for (i = 0; i < len; i++) {
		...
		//printf ("doMFToString, element %d is %d\n",i,_v);
			 ...
			 uintN argc, jsval *argv, jsval *rval)
{
	 ...
	 */
	//printf ("start of SFNodeConstr, obj %d argc %d\n",obj,argc);
...
SFNodeGetProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
	...
	if (JSVAL_IS_INT(id)) {
		...
		//printf ("getting property for vuff %s\n",_buff);
