 ...
 * 
 * $Id: jsVRMLClasses.c,v 1.18 2003/04/08 18:15:43 ayla Exp $
 * 
 */
...
doMFToString(JSContext *cx, JSObject *obj, const char *className, jsval *rval)
{
	...
	if (!memcmp(className, "MFString", className_len)) {
		isString = JS_TRUE;
	}
...
doMFAddProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
	...
	if (!memcmp(p, "length", p_len) || !memcmp(p, "toString", p_len) ||
		!memcmp(p, "__touched_flag", p_len) || !memcmp(p, "assign", p_len) ||
		!memcmp(p, "constructor", p_len)) {
		if (verbose) {
